...
import {
  recomputeAllUserTotals,
  checkChampionBonuses,
...
export async function GET(req) {
  ...
  for (const m of data.matches) {
    ...
    const externalRef = isKnockout
      ? `wc26-${m.num}`
      : `wc26-${m.round}-${m.team1}-${m.team2}-${m.date}`;

    const isRealTeamName = (s) => !!s && !/^[0-9WL]/.test(s) && s.length > 1;

    const homeTeam = isRealTeamName(m.team1)
      ? await getOrCreateTeam(m.team1, m.group)
      : null;

    const awayTeam = isRealTeamName(m.team2)
      ? await getOrCreateTeam(m.team2, m.group)
      : null;
    ...
    if (justFinished) {
      finishedNow.push(match.id);
    }
  }

  let predictionsScored = 0;
  const stagesToCheck = new Set();
  ...
  if (finishedNow.length > 0) {
    ...
    for (const stage of stagesToCheck) {
      await generateRewardWinnerForStage(stage);
    }
  }
  ...
  return NextResponse.json({
    ...
    predictionsScored,
    stagesCheckedForRewards: Array.from(stagesToCheck),
  });
}